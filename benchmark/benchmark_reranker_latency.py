#!/usr/bin/env python3
"""
Reranker Model Latency Benchmark

다양한 리랭커 모델의 레이턴시를 측정합니다.
vLLM의 LLM(runner="pooling")을 사용하여 직접 로드하고 score()로 측정합니다.
...
Usage:
    # 모든 모델 벤치마크
    python benchmark/benchmark_reranker_latency.py

    # 특정 모델만
    python benchmark/benchmark_reranker_latency.py --model BAAI/bge-reranker-v2-m3
...
# hf_overrides: Qwen3 reranker 모델은 CausalLM/VL arch → SequenceClassification으로 변경 필요
# bge-reranker-v2-m3는 native XLMRobertaForSequenceClassification이므로 override 불필요
...
def prepare_query_doc_pairs(
    ...
    doc_length: int,
    count: int,
    query_length: int = QUERY_TOKEN_TARGET,
    stride: int = 100,
) -> tuple[list[str], list[str]]:
    """Create `count` (query, document) pairs.

    Query: fixed ~query_length tokens.
    Document: exactly doc_length tokens.
    Each pair uses different content via staggered offsets.
    """
    queries: list[str] = []
    documents: list[str] = []
    offset = 0
    ...
    for _ in range(count):
        # Query
        if offset + query_length > len(source_tokens):
            offset = 0
        q_segment = source_tokens[offset:offset + query_length]
        q_text = tokenizer.decode(q_segment, skip_special_tokens=True)
        ...
        if offset + doc_length > len(source_tokens):
            offset = 0
        d_segment = source_tokens[offset:offset + doc_length]
        d_text = tokenizer.decode(d_segment, skip_special_tokens=True)
        # Verify round-trip token count; trim if boundary caused mismatch
        re_encoded = tokenizer.encode(d_text, add_special_tokens=False)
        if len(re_encoded) != doc_length:
            d_text = tokenizer.decode(re_encoded[:doc_length], skip_special_tokens=True)
...
def benchmark_model(
    ...
    """Benchmark a single reranker model using llm.score()."""
    ...
    override_str = f", hf_overrides={hf_overrides}" if hf_overrides else ""
    print(f"  Loading model with vLLM (runner=pooling, max_model_len={model_max_len}{quant_str}{override_str}) ...")
    ...
    for length in effective_lengths:
        for batch_size in batch_sizes:
            ...
            try:
                total_pairs_needed = batch_size * (num_warmup + num_runs)
                all_queries, all_documents = prepare_query_doc_pairs(
                    tokenizer, source_tokens, length, total_pairs_needed,
                )
                pair_idx = 0

                # Warmup
                for _ in range(num_warmup):
                    q_batch = all_queries[pair_idx:pair_idx + batch_size]
                    d_batch = all_documents[pair_idx:pair_idx + batch_size]
                    pair_idx += batch_size
                    llm.score(q_batch, d_batch)
                ...
                # Throughput: total document tokens processed per second
...
def print_summary_table(all_results: list[dict]):
    ...
    print("\n" + "=" * 110)
    print("SUMMARY: Reranker Model Latency Comparison")
    print("=" * 110)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Benchmark reranker model latency with vLLM",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="vLLM quantization method (e.g. 'compressed-tensors'). None for BF16.",
    )
    parser.add_argument(
        "--model-dir",
        type=str,
...
def main():
    ...
    # Filter models
    if args.model:
        models = [m for m in RERANKER_MODELS if m["model_id"] == args.model]
        if not models:
            print(f"ERROR: Model '{args.model}' not found in RERANKER_MODELS.")
            print(f"Available: {[m['model_id'] for m in RERANKER_MODELS]}")
            sys.exit(1)
    else:
    ...
    print("=" * 70)
    print(f"Reranker Model Latency Benchmark ({quant_label.upper()})")
    print("=" * 70)
    ...
    for i, model_cfg in enumerate(models):
        ...
        print(f"  max_context={model_cfg['max_context']}")
        if model_cfg.get("hf_overrides"):
            print(f"  hf_overrides={model_cfg['hf_overrides']}")
        print(f"{'─' * 70}")
    ...
    default_output = os.path.join(
        ...
        f"reranker_latency_results_{quant_label}.json",
    )
